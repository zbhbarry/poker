...
public class Desk {
            ...
            do {
                for (Player player : players) {
                    ...
                    if (!(player.getIsAllIn() == 1 || player.getIsFold() == 1)) {
                        if (player instanceof AiAgent aiAgent) {
                            double[] state = getState(aiAgent);
                            Player.Action action = null;
                            if(aiAgent instanceof DqnAgent dqnAgent) {
                                action = dqnAgent.SelectAction(state, bet.GetValidActions(aiAgent));
                            }
                            Set<Player.Action> actions = new HashSet<>(bet.GetValidActions(aiAgent));
                            if (action != null) {
                                bet.DoAction(action, aiAgent);
                            }
                            addExperience(state, aiAgent, action, actions);
    ...
    //添加步骤
    private void addExperience(double[] state, AiAgent aiAgent, Player.Action action, Set<Player.Action> validActions)
    {

        if(!aiAgent.getExperiences().isEmpty()){
            aiAgent.getExperiences().get(aiAgent.getExperiences().size()-1).setNextStates(state);
        }
        aiAgent.getExperiences().add(new Experience(state, action, null,validActions));
    }

    private double[] getState(AiAgent aiAgent){
    ...
    private void setReward() {
        ...
        */
        for (Player player : players) {
            if(player instanceof AiAgent aiAgent) {
                double reward = State.roundToThreeDecimalPlaces((double) aiAgent.getChips() / aiAgent.getOriginChips() - 1);
                //完善最终的状态，设置是否完成标志
                Experience exp = aiAgent.getExperiences().get(aiAgent.getExperiences().size() - 1);
        ...
        for (Player player : players) {
            if(player instanceof AiAgent aiAgent) {
                if (aiAgent.getExperiences().get(aiAgent.getExperiences().size() - 1).getReward() > 0) {
                    aiAgent.setWinCount(aiAgent.getWinCount() + 1);
                }
            }else {
